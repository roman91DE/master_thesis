...
Besides studying the performance for the airfoil dataset, additional real world symbolic regressions experiments used the datasets[@machine_learning_repo] described in table \@ref(tab:full-run-realWorldSymReg-problems).
...
```{r first-gen-airfoil-byNeurons, echo=FALSE, fig.cap="First Generation Median Training Error for variable number of hidden Neurons", out.width="90%"}
...
```{r first-gen-airfoil-byLayers, echo=FALSE, fig.cap="First Generation Median Training Error for variable number of hidden Layers", out.width="90%"}
...
To conclude the first experiment, figure \@ref(fig:airfoil-popDiversity) shows the median population diversity measured by the normalized Levenshtein edit distance for both DAE-GP algorithms and regular GP as a benchmark. While the population diversity steadily decreases over the generations for standard GP, DAE-GP variants strongly increase the population diversity in the first generations and keep them at a high level of over $0.9$. Here it can be observed, that pre-training shows a positive effect on the results by increasing population diversity in comparison to regular DAE-GP.
...
Since previous research on DAE-GP used a single hidden layers for the airfoil dataset [@dae-gp_2022_symreg], I repeated the experiment with a single hidden layer. Based on the findings of @pmlr-v5-erhan09a, I expected that pre-training DAE-GP would lead to a decrease in overall performance. Figures \@ref(fig:airfoil-fitness-perGen) and \@ref(fig:airfoil-finalFitness) show the average best fitness per generation as well as the distribution of final fitness scores after 30 generations. The results are, again, based on $10$ individual runs per algorithm. As expected it shows, that for a small DAE-LSTM size pre-training negativley impacts the fitness of solution found by DAE-GG.


```{r airfoil-fitness-perGen, echo=FALSE, fig.cap="Fitness over 30 Generations - Airfoil (1HL)", out.width="80%"}
knitr::include_graphics("./img/airfoil_1hl_maxIndSize_fullRun_30gens/mean_median_fitness_byGens.png")
```

```{r airfoil-finalFitness, echo=FALSE, fig.cap="Fitness after 30 Generations - Airfoil (1HL)", out.width="80%"}
knitr::include_graphics("./img/airfoil_1hl_maxIndSize_fullRun_30gens/final_fit_boxplot.png")
```
...
To gather more confidence in the previous results, the pre-training strategy for DAE-GP was tested for three additional real world symbolic regression problems taken from @machine_learning_repo.^[see table \@ref(tab:full-run-realWorldSymReg-problems)]
...
```{r altStrategies-Fitness, echo=FALSE, fig.cap="Best Fitness over 30 Generations - Alternative Pre-Trategies - Airfoil", out.width="80%"}
...
```{r altStrategies-FitnessDistr, echo=FALSE, fig.cap="Best Fitness after 30 Generations - Alternative Pre-Trategies - Airfoil", out.width="80%"}
