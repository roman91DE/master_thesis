...
If we look at another time consuming operation in DAE-GP, the creation of new individuals by sampling the DAE-LSTM model $M_G$, we see one main reason for the large runtime increase by pre-training DAE-GP. Figure \@ref(fig:airfoil-SamplingTime) shows the median sampling time spend during each generation of the search and it clearly shows that pre-training does strongly increase the time spent on model sampling in comparison to regular DAE-GP 
