...
```
To test for statistical differences between the distributions of results achieved by either pre-trained or regular DAE-GP, I selected the  
nonparametric Mann-Whitney-U Test ([@mwu1], [@mwu2]) to test if the distribution .... Additionaly the effect strength of pre-training is computed by using Cliffs Delta[@Cliff1993DominanceSO].
...
```{r symReg-Fitness, echo=FALSE, fig.cap="Fitness over 30 Generations - Summary - Real World Symbolic Regression", out.width="80%"}
knitr::include_graphics("~/masterThesis/master_thesis/img/symbolicRegressionSummarized/median_fitness_byGens.png")
```


Table \@ref(tab:full-run-realWorldSymReg-fitness) summarizes the results for the median best fitness achieved after 30 generations and shows the results of computing the statistical tests. Higher fitness values (lower RMSE) are printed bold and p-values are marked with asterisk (1,2,3) for $\alpha$ levels ($0.1$, $0.05$, $0.01$). 
...
knitr::kable(
 ...
 col.names = col_names,
 align = "l",
 caption = "Median Best Fitness after 30 generations - Summary - Real World Symbolic Regression"
) %>% row_spec(0,bold=TRUE ) %>% kableExtra::kable_styling(latex_options = "hold_position")
...
```{r symReg-Size, echo=FALSE, fig.cap="Size of the best Solution over 30 Generations - Summary - Real World Symbolic Regression", out.width="80%"}
...
knitr::kable(
 ...
 col.names = col_names,
 align = "l",
 caption = "Median Size of the best solution after 30 generations - Summary - Real World Symbolic Regression"
