...
Even though a majority of EDA algorithms are designed to work with fixed length string representations as used in classical GAs, EDA algorithms have been researched an successfully applied to the domain of GP in various different research streams.
...
The results of the Mann-Whitney-U Test again show, that differences in the underlying distributions are not statistically significant which also shows in low effect strengths of using pre-training on the size of the best found solutions (Cliff's Delta ranges from`r min(df_size$Cliffs_Delta)` to `r max(df_size$Cliffs_Delta)`).
...
Looking at the effect strength of pre-training, it shows that pre-training does have a strong positive effect (Cliff's Delta ranges from `r min(df_popDiv$Cliffs_Delta)` to `r max(df_popDiv$Cliffs_Delta)`) on population diversity inside the population.
...
Analyzing the effect strength of pre-training on the number of training epochs shows values for Cliff's Delta in the range of `r min(df_epochs$Cliffs_Delta)` and `r max(df_epochs$Cliffs_Delta)` which indicates a very strong negative effect on the number of training epochs necessary at each generation of the search.
