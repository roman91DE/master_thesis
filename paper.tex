...
\author{}
\date{\vspace{-2.5em}Date of Submission: 2023-02-02}
...
\caption{Best Fitness over 30 Generations - Alternative Pre-Strategies - Airfoil}\label{fig:altStrategies-Fitness}
\end{figure}
...
\caption{Best Fitness on Test Set after 30 Generations - Alternative Pre-Trategies - Airfoil}\label{fig:altStrategies-FitnessDistr}
\end{figure}
...
Suprisingly, even after correcting the runtime for both DAE-GP results by removing the time spend during sampling, my implementation of pre-training for DAE-GP still on average takes more time to finish the search process as shown in figure \ref{fig:airfoil-runtime-noSampling-total}. While traditional DAE-GP has a median runtime without sampling time of 3550.277 seconds , the runtime of pre-trained DAE-GP, without time for samling and pre-training, still increases by a factor of 1.865 to 6622.027 seconds. While investigating other reasons for the differences in runtime, I verified that both DAE-GP variants use the exact same number of trainable parameters for each generations DAE-LSTMS models ( NULL).
...
To gather more confidence in the previous results, the pre-training strategy for DAE-GP was tested for three additional real world symbolic regression problems taken from {[}\protect\hyperlink{ref-machine_learning_repo}{2}{]}. Table \ref{tab:full-run-realWorldSymReg-problems} briefly summarizes the benchmark problems.
