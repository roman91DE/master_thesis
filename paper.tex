...
AE were introduced by {[}\protect\hyperlink{ref-ae_orig}{6}{]} to compress high-dimensional data into lower-dimensions. An AE consists of two different sub-units:
...
The AE is trained to reduce the reconstruction error between input and output data, after the training procedure is finished the network is able to reduce the dimensional of input data to a compressed representation{[}\protect\hyperlink{ref-ae_orig}{6}{]}.
...
One difficulty in the implementation of a pre-training strategy into DAE-GP has been the determination of the number of hidden neurons in side the DAE-LSTMs hidden layers. The original description of DAE-GP {[}\protect\hyperlink{ref-dae-gp_2020_rtree}{18}{]} uses a strategy where the number of hidden neurons for each hidden layer is dynamically set per generation to the maximum individual size inside the current population.
...
To ensure a fair comparison between pre-trained and regular DAE-GP, the prior experiments used a static number of hidden neurons for each hidden Layer inside the DAE-LSTM networks. For comparison, figure \ref{fig:airfoil-maxIndSize-params} shows the total number of trainable parameters for each generation's DAE-LSTM of another experiment where regular DAE-GP was tested with dynamic adjustment of the number of hidden neurons as described in {[}\protect\hyperlink{ref-dae-gp_2020_rtree}{18}{]} and {[}\protect\hyperlink{ref-dae-gp_2022_symreg}{17}{]}. Again, the experiments was based on \(10\) individual runs and otherwise used the same hyperparameters as the previous one. It clearly shows that DAE-GP can strongly reduce the number of trainable parameters by adjusting the depth of it's hidden layers to the current generations population which will further reduce the amount of training necessary and likely reduce the run-time of DAE-GP.
...
\caption{Median Number of trainable Parameters over 30 Generations - Airfoil (2HL)}\label{fig:airfoil-maxIndSize-params}
\end{figure}
...
\caption{Median Best Fitness over 30 Generations - Airfoil (2HL) with dynamic adjustment of regular DAE-GP}\label{fig:airfoil-maxIndSize-fitness}
\end{figure}
...
\ldots{} describe differences in distribution, interquartile range blablabla\ldots{}
...
{[}\protect\hyperlink{ref-pmlr-v5-erhan09a}{3}{]}: Pre-Training effect especially useful for lower-level layers -\textgreater{} Only adapt weights from low layers or embed the pre-trained model?
