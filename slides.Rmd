...
## Zusammenfassung Experimente zur symbolischen Regression
...
df <- read.csv("/Users/rmn/github/master_thesis/data/summary_table_final_fit.csv", sep=",")
print(df)

# Create a function to format the values
bold_min <- function(x, y){
  if(x <= y) return(paste0("**", x, "**"))
  else return(x)
}

# Apply the function to the data frame
df$DAE.GP <- mapply(bold_min, df$DAE_GP, df$Pre_Trained_DAE_GP)
)
...
)%>% row_spec(0,bold=TRUE ) %>% kableExtra::kable_styling(latex_options = "hold_position", font_size = 7)
```
